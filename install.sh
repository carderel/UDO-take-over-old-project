    ...
    if [ -n "$CONFLICTS" ]; then
        ...
        case $REPLY in
            ...
            [Ff])
                ...
                rm -rf ORCHESTRATOR.md PROJECT_STATE.json PROJECT_META.json CAPABILITIES.json .agents .templates .project-catalog .inputs .outputs .checkpoints .rules .memory START_HERE.md LESSONS_LEARNED.md HARD_STOPS.md NON_GOALS.md OVERSIGHT_DASHBOARD.md .udo-version 2>/dev/null || true
                UPDATE_MODE=false
                ;;
            *)
                echo "Cancelled."
...
# ============================================================
# HARD_STOPS.md - Absolute rules that are NEVER violated
# ============================================================
if [ ! -f "HARD_STOPS.md" ]; then
cat > HARD_STOPS.md << 'HARDSTOPSEOF'
# Hard Stops
...
├── LESSONS_LEARNED.md           # Mistakes to avoid (Layer 3)
├── NON_GOALS.md                 # What this system is NOT for
├── OVERSIGHT_DASHBOARD.md       # Human monitoring interface
│
...
When user indicates something was wrong or corrects you:
...
Before adding, scan existing lessons for potential conflicts:
```
⚠️ Potential Conflict Detected

Existing lesson (L003): "Always use tabs for indentation"
New lesson: "Always use spaces for indentation"

Options:
1. Replace old with new
2. Keep old, discard new  
3. Both valid in different contexts (specify when each applies)
```

### Step 4: Determine Layer
- Is it absolute (security, legal, compliance)? → Suggest adding to HARD_STOPS.md (user must edit)
- Is it a stable standard? → Add to `.rules/` file
- Is it agent-specific? → Add to agent template under `## Learned Rules`
- Is it situational/recent? → Add to LESSONS_LEARNED.md

### Step 5: Confirm
"Added [lesson title] to [location]. 
- Layer: [0/1/2/3]
- Scope: [where it applies]
- Conflicts checked: None found"

### Step 6: Track
If lesson applies to an agent, update the Agent Lesson Index in LESSONS_LEARNED.md.
...
### Automatic Triggers
- 20 lessons in LESSONS_LEARNED.md → Prompt: "Time to review and graduate lessons?"
- 15 rules in single agent template → Prompt: "Agent has many rules. Review for consolidation?"
- 30 days since last review → Suggest lesson audit

### Review Process
1. List all lessons with date added
2. For each: Still relevant? Still correct? Applied successfully?
3. Graduate stable lessons to higher layers
4. Archive deprecated lessons
5. Merge similar lessons
6. Update Agent Lesson Index
...
When starting fresh:
1. Read HARD_STOPS.md first (absolute rules)
2. Ask user to fill in `CAPABILITIES.json` for their environment
3. Ask for the project goal
4. Ask clarifying questions for `PROJECT_META.json`
5. Review `NON_GOALS.md` - confirm project fits UDO's purpose
...
2. When adding a lesson:
   - Is it agent-specific? → Add to that agent's `## Learned Rules` section
   - Is it a stable standard? → Add to appropriate `.rules/` file
   - Is it situational/recent? → Add here
3. When lessons pile up, prompt user to review and graduate stable ones to Layer 1 or 2

**For Humans**: 
- When you correct the AI, say "add to lessons"
- AI will ask clarifying questions before adding
- Review this file periodically to graduate stable lessons upward

---

## Agent Lesson Index

Track which agents have learned rules:
...
Applied 5+ times successfully?
     ↓
YES → Graduate to Layer 2 (agent template) or Layer 1 (.rules/)
     ↓
Remove from this file (it's now permanent)
```
...
Lessons that were graduated or deprecated:
...
echo ""
echo "Folders:"
echo "  .agents/                   - Specialist definitions (Layer 2)"
echo "  .rules/                    - Quality standards (Layer 1)"
