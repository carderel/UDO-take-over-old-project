    ...
    switch ($choice.ToLower()) {
        ...
        'f' {
            ...
            $itemsToRemove = @(
                ...
                "START_HERE.md", "LESSONS_LEARNED.md", "HARD_STOPS.md", "NON_GOALS.md", 
...
- **HS-DATA-001**: NEVER store PII in logs or outputs
...
1. **Is it truly absolute?** - If it needs "except when..." it belongs in .rules/
2. **Is it serious?** - Security, legal, compliance, or client-critical only
3. **Is it rare?** - Keep total hard stops under 15
...
If AI detects it is about to violate a hard stop:
...
```
'@
Set-Content -Path "HARD_STOPS.md" -Value $HARD_STOPS -Encoding UTF8
...
Write-Host ""
Write-Host "Folders:"
Write-Host "  .agents/                   - Specialist definitions (Layer 2)"
Write-Host "  .rules/                    - Quality standards (Layer 1)"
